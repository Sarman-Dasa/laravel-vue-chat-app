    ...
    <div>
        <div class="flex flex-col justify-end h-80 ">
            <div class="flex items-center justify-between p-2">
                ...
                <!-- <button type="button" class="inline-flex justify-center rounded-md bg-gray-600 px-3 py-1 text-sm font-semibold text-white shadow-sm hover:bg-gray-500 sm:ml-3 sm:w-auto" @click="open = false">
                Deactivate
                </button> -->
                ...
                <div v-for="item in messageList" :key="item.id" class="flex items-center mb-2">
                    <div v-if="item.sender_id === currentUser.id" class="relative ml-auto p-2 rounded-lg message"
                        ...
                        <div v-if="editingMessageId !== item.id">
                            ...
                            <span v-if="item.is_edited" class="absolute bottom-2 right-2 edited-message">Edited</span>
                            <button v-if="isEditableMessage(item.created_at)"
                            ...
                            <button class="right-0 top-0 ml-1 editBtn" @click="deleteMessage(item.id)">
                                <svg fill="#FA5252" xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 16 16" width="16px"
                                    height="16px">
                                    <path
                        ...
                        <input v-if="editingMessageId === item.id" type="text" v-model="updatedMessage"
                            @keyup.enter="updateMessage" @blur="cancelEdit"
        ...
        <div class="flex items-center">
            <input type="text" v-model="message" @keydown="sendTypingEvent" @keyup.enter="sendMessage"
